...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,l_files,n_files,path,file_name_sat,file_name_mod,batch_size=1):
        super().__init__()
    ...
    def __getitem__(self,i):
        ...
        if self.batch_size*(i_2+1) <= self.l_files:
            return tuple([d_sat[i_2*self.batch_size:self.batch_size*(i_2+1)],d_mod[i_2*self.batch_size:self.batch_size*(i_2+1)]])
        else:
            return tuple([d_sat[i_2*self.batch_size:],d_mod[i_2*self.batch_size:]])
